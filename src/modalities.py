...
class modality:
    def __init__(self, path=None , mat_type="gaussian"):
        ...
        print("Set modality parameters: similarity-matrix degree-matrix laplacian-matrix")
...
class lap_list:
    def __init__(self , lap=None , rank=None):
        ...
        self.orthonorm_basis = helpers.utils.get_orthonorm_basis(lr_list = self.lra, rank = rank) 
        self.H = helpers.utils.get_H_matrix(orthonorm_basis = self.orthonorm_basis , lr_list = self.lra , chi_list = self.chi_list , rank = rank)
        self.rotation = self.__compute_rotation_matrix()
        self.joint_eig_vectors = self.__compute_eig_vectors()
        print("Set laplacian-list parameters: rank M low-rank-approximations relevance sorted-laplacian orthonormal-basis-matrix H-matrix rotation-matrix joint-eigenvector-matrix")
