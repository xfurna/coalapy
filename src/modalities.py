...
class lap_list:
    def __init__(self , lap=None , rank=None):
        ...
        self.M = len(lap)
        self.lap = lap
        self.lra = self.__compute_lra()
        self.weights = helpers.utils.get_weights(self.lap)
        self.orthonorm_basis = helpers.utils.get_orthonorm_basis(lr_list = self.lra, rank = rank) 
        self.H = helpers.utils.get_H_matrix(orthonorm_basis = self.orthonorm_basis , lr_list = self.lra, rank = rank)
